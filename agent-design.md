...
This document describes the design philosophy and general architecture of scoped AI agents built for a single user. The system currently includes three agents: **Cara**, a general-purpose executive function and life assistant that serves as the core model for this project. Also currently included as an example models is **Erika**, a language-learning assistant.

Each agent is built to work with a general-purpose LLM (via OpenRouter) and differentiated entirely through prompting — no fine-tuning, no custom model weights. The work is in the document layer.
...
This remains a work in progress, and these instructions sometimes fail. Since Claude models are popular and do well in RLHF, it's an important consideration going forward and it requires explicit instruction reinforcement in the identity layer. The framing rules in Cara's identity document in particular represent the current best-effort solution; they will also require occasional recalibration as model behavior shifts across versions.
...
- **Freeze is the primary blocker, not motivation.** Agents are designed around the assumption that when a user cannot initiate, the cause is nervous system state — not willingness, not laziness, not a motivational deficit that needs to be addressed through encouragement or urgency.
...
- **Project instructions** — the agent's primary operating definition: role, tone, mode framework, global rules, and language constraints. This lives in the platform's system prompt layer and loads with every session.
...
- **Profile and context documents** — user-specific knowledge the agent needs to do its job: user profile, active projects, domain expertise, session history. Depending on the size of these documents, token length and budgeting may be a factor that changes your project architecture. See [GITHUB LINK]context-management.md for more discussion on this topic.

The identity document is the system's learning mechanism. Because the LLM has no persistent memory between sessions, corrections can't accumulate organically — they have to be written down and re-fed. The identity doc is where that happens. This allows for an agent that can grow with interactions with a user, ideally minimizing threat perception, increasing engagement with the agent, and ultimately serving the primary goal of user uplift toward enhanced productivity.

## Core Agent - Cara

As the core assistant, Cara was the first designed, aimed to assist users experiencing executive dysfunction arising from hypoarousal states. This includes, but is not limited to, conditions such as ADHD (particularly inattentive presentations) and PTSD freeze-type responses. In these presentations, the limiting factor is rarely motivation or capability — it is nervous system state. The user's capacity to initiate, sustain, or complete tasks is modulated by perceived threat level, with visibility, evaluation, and requests for help all functioning as potential threat signals. Ultimately, Cara is the protoype for more specialized agents in this project (such as Erika).

[add details on how she helps plan a calendar, or helps prepare for a given task.]

### Design Constraints

The core constraint Cara is built around: **the user's nervous system may read visibility, evaluation, and asking for help as threat.** The agent is designed around reducing that threat load, not around motivating, pushing, or optimizing.
...
- CBT/DBT-style pattern challenges
...
| **Handoff** | Thread is ending | Write session log, update profile documents |
...
The distinction is subtle but load-bearing. Imperative sentences — even gentle ones — carry an implicit evaluator. Observation sentences don't. For a user whose nervous system treats evaluation as threat, the difference is real.

User feedback has indicated short replies are the highest-risk case. Any reply under five sentences gets a final-sentence audit before sending: if the last sentence starts with a verb or sounds like an instruction, it gets rewritten.

### Identity Document as Learning Layer

`cara-identity.md` holds corrections and promoted rules from previous sessions. 

Rules promote from single corrections to the identity doc when they've been corrected twice. They promote from the identity doc to space instructions at significant revision cycles. This might include user reaction to specific language the model has used, normal work hours (a user might be on a third shift schedule, for example, and some LLMs might incorrectly flag this as a time for rest), preferred work pace during the day, etc. This serves as a frictionless adaptive mechanism as the model grows with the user. A future onboarding feature may help to mitigate initial friction with the agent until promoted rules are in place.

## Specialized Agent Examples #1 - Erika

Erika is a Japanese language learning assistant scoped to a single user working on Japanese language study, a standardized exam for non-native speakers that recognizes proficiency. She handles grammar questions, vocabulary, reading, and study strategy.

### Design Constraints

Erika's design is shaped by three constraints:

**The i+1 principle.** Every explanation is calibrated just above the user's current comprehension level — challenging enough to acquire new material, not so far ahead as to overwhelm. Krashen's input hypothesis is the explicit theoretical frame. LLMs are well-suited to execute this principle, as output can be calibrated in real time to the user's growth.

**Responsiveness to English-to-Japanese production.** LLMs are excellent for training by way of language production, as they won't feel social pressure to steer away from correcting a user when appropriate. This can be a delicate balance, as a depleted user may need a lighter touch.

**Neurological state-dependent learning.** On low-capacity days, Erika shifts away from new acquisition toward consolidation and review. New grammar acquisition requires executive bandwidth that isn't always available. Reading the room is an explicit part of Erika's operating instructions — if the user seems depleted, suggest review without making it a big deal.

### User Profile Integration

Erika's identity document contains a detailed learner profile: current textbook, Anki deck and daily card volume, relative skill levels across modalities (reading stronger than listening; recognition stronger than production), and active weak spots being tracked across sessions.

Interest areas are part of the operating context — not just as flavor, but as a calibration resource. Example sentences, grammar context, and content recommendations are drawn from media the user actually engages with [insert user preferences here]. This makes the calibration real rather than generic.
...
Space instructions define the agent's operating behavior. The identity document tracks what's been learned and corrected. They're separate files because they serve different functions and update on different cycles.

Space instructions change slowly — when operating parameters shift in a meaningful, stable way. The identity document changes at every session handoff, capturing new corrections before they're forgotten.

### Corrective Feedback Loop

The handoff mode (Cara) and implicit session-end review (Erika) both exist to capture corrections while they're still fresh. Without a write-back mechanism, corrections that happen mid-session don't survive to the next one. The identity document is that mechanism.

This is a workaround for statelessness, not a solution to it. The agent still starts each session from scratch — this is a core conceit of LLM design — but it starts from a richer scratch because relevant corrections have been pre-loaded.
